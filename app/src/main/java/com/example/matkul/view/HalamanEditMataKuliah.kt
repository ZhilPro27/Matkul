...
@Composable
fun EditMataKuliahScreen(
    navigateBack: () -> Unit,
    onNavigateUp: () -> Unit,
    modifier: Modifier = Modifier,
    viewModel: MataKuliahEditViewModel = viewModel(factory = PenyediaViewModel.Factory)
) {
    val coroutineScope = rememberCoroutineScope()
    var deleteConfirmationRequired by rememberSaveable { mutableStateOf(false) }

    // 1. Ambil daftar prodi agar dropdown di form bisa muncul
    val listProdi by viewModel.dataProgramStudi.collectAsState()

    Scaffold(
        topBar = {
            MatkulTopAppBar(
                title = DestinasiEditMataKuliah.tittleRes,
        ...
        Column(
            ...
            // 2. Kirim listProdi ke Body
            EntryMataKuliahBody(
                uiStateMataKuliah = viewModel.uiStateMataKuliah,
                listProdi = listProdi, // Tambahkan ini
